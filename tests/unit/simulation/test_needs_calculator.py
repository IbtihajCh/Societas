...
class TestDecayNeeds:
    ...
    def test_decay_sleep_auto_replenish(self) -> None:
        """Sleep decays by SLEEP_DECAY_RATE but replenishes by SLEEP_REPLENISH_RATE."""
        agent = _make_agent()
        ...
        expected = 0.6 - SLEEP_DECAY_RATE + SLEEP_REPLENISH_RATE
    ...
    def test_wealth_class_derivation(self) -> None:
        """derive_wealth_class returns POOR for <1000, MIDDLE for <15000, RICH otherwise."""
        assert derive_wealth_class(0.0) == WealthClass.POOR
        assert derive_wealth_class(999.99) == WealthClass.POOR
        assert derive_wealth_class(1000.0) == WealthClass.MIDDLE
        assert derive_wealth_class(14999.99) == WealthClass.MIDDLE
        assert derive_wealth_class(15000.0) == WealthClass.RICH
...
class TestDeriveWealthClass:
    ...
    def test_middle(self) -> None:
        assert derive_wealth_class(7500.0) == WealthClass.MIDDLE
        assert derive_wealth_class(14999.99) == WealthClass.MIDDLE
...
class TestEdgeCases:
    ...
    def test_health_not_modified_by_decay(self) -> None:
        """decay_needs does not modify agent.resources.health."""
